...
const _sessionStorageStore: Record<string, string> = {};
const sessionStorageMock: Storage = {
  getItem(key: string): string | null { return Object.prototype.hasOwnProperty.call(_sessionStorageStore, key) ? _sessionStorageStore[key] : null; },
  setItem(key: string, value: string): void { _sessionStorageStore[key] = String(value); },
  removeItem(key: string): void { delete _sessionStorageStore[key]; },
  clear(): void { Object.keys(_sessionStorageStore).forEach(k => { delete _sessionStorageStore[k]; }); },
  get length(): number { return Object.keys(_sessionStorageStore).length; },
